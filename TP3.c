...
char** CriaGrafo(int *tam){
    ...
    */
    fgets (str,10,input); 
    for (i = 0; str[i]!= '\n'; i++){ // Ler do arquivo quantas empresas tentaram ser instaladas ao mesmo tempo
        *tam = *tam + (str[i] - '0') * mult;  // Converter de string para inteiro
        mult *= 10;
    }
    ...
    while (!feof(input)){ // Ler cada linha do arquivo para criar os caminhos que existem entre as empresas
        ...
        vert2 = 0;
        for (i = 0; str[i] != '\0'; i++){
            if (str[i+1] == '\n'){ // Se ja tiver lido um valor, guarda o segundo, indica que a matriz chegou ao fim
                for (j = i; str[j] != 32; j--){
                    vert2 += (str[j] - 48) * mult;
                    mult*=10;
                }
            }
            if (str[i] == 32){ // Guarda o primeiro valor lido no arquivo
                for (j = i - 1; j >= 0; j--){
                    vert1 += (str[j] - '0') * mult;
                }
            }
